...
export class AgentService {
  ...
  async register({ agent_id, agent_type = 'generic', metadata = {}, webhook_url, webhook_secret, seed, public_key, tenant_id }) {
    ...
    // Reserve scheme-like prefixes to prevent spoofing system-generated identifiers.
    // DIDs (did:*) are generated by the server; agent: was the legacy URI scheme.
    // Bare IDs that start with these prefixes would be indistinguishable from
    // system-generated ones in storage lookups and audit logs.
    if (/^(did:|agent:)/i.test(agent_id)) {
      throw new Error('agent_id may not start with reserved prefixes "did:" or "agent:"');
    }
