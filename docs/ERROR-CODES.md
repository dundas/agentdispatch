...
| Code | HTTP | Retryable | Description | Hint |
|------|------|-----------|-------------|------|
| `REGISTRATION_FAILED` | 400 | No | Agent registration failed | Check `agent_id` uniqueness, length (max 255 chars, checked before regex), character set (`^[a-zA-Z0-9._\-:]+$`), and required fields |
...
- **`REGISTRATION_FAILED` agent_id validation**: If you receive this because of an invalid `agent_id`, check both constraints: the `agent_id` must be 255 characters or fewer (checked first) AND match `^[a-zA-Z0-9._\-:]+$`. Slashes, spaces, and `agent://` prefixes are not valid agent IDs.
