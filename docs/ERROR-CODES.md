...
| Code | HTTP | Retryable | Description | Hint |
|------|------|-----------|-------------|------|
| `REGISTRATION_FAILED` | 400 | No | Agent registration failed | Check `agent_id` uniqueness, character set (`^[a-zA-Z0-9._\-:]+$`), and required fields |
...
- **`REGISTRATION_FAILED` agent_id validation**: If you receive this because of an invalid `agent_id`, fix the character set — only `^[a-zA-Z0-9._\-:]+$` is allowed. Slashes, spaces, and `agent://` prefixes are not valid agent IDs.
