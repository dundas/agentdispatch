...
|-------|------|----------|-------------|
| `agent_id` | string | No | Custom agent ID. Must be 255 characters or fewer AND match `^[a-zA-Z0-9._\-:]+$`. Length is checked first (O(1) guard before regex). May not start with `did:` or `agent:` (reserved prefixes). If omitted, server generates `agent-<uuid>`. |
